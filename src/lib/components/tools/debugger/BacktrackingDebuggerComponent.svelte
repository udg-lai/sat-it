...
<div class="pack mb-1 flex flex-row items-center gap-2">
	...
	{#if isJust($followingVariable)}
		...
		<input
			type="number"
			class="variable-selector flex-grow min-w-0"
		...
		/>	
	{:else}
		<span class="flex-grow min-w-0">No more variables to assign</span>
	{/if}

	<div class="w-[4.5rem] flex flex-col justify-between shrink-0 gap-1">
