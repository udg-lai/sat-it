...
function parseSummary(lines: string[]): Either<ErrorMessage, Tuple<number, number>> {
    ...
    if (cnf !== "cnf") {
        return makeLeft(`[ERROR]: boolean formula expected to be represented in Conjuntive Normal From (CNF)`)
    }
    const varsCount = parseInt(vars)
    if (Number.isNaN(varsCount)) {
        return makeLeft(`[ERROR]: could not parse expected variable count to number representation of ${vars}`)
    }
    const clausesCount = parseInt(clauses)
    if (Number.isNaN(clausesCount)) {
        return makeLeft(`[ERROR]: could not parse expected clauses count to number representation of ${clauses}`)
    }
    return makeRight(makeTuple(varsCount, clausesCount))
}

function parseCNF(lines: string[], varCount: number, clauseCount: number): Either<ErrorMessage, number[][]> {
    if (lines.length > clauseCount) {
        return makeLeft(`[ERROR]: number of CNF greater than the number of expected clauses ${clauseCount}`)
    }
    const cnf = lines.map(line => line.trim().split(" ").map(n => parseInt(n)))
