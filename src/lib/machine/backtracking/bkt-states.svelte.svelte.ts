...
const bkt_state: NonFinalState<BKT_BACKTRACKING_FUN, BKT_BACKTRACKING_INPUT> = {
	id: bkt_stateName2StateId['bkt_state'],
	run: backtracking,
...
const decision_level_state: NonFinalState<BKT_DECISION_LEVEL_FUN, BKT_DECISION_LEVEL_INPUT> = {
	...
	transitions: new Map<BKT_DECISION_LEVEL_INPUT, number>()
		.set('backtracking_state', bkt_stateName2StateId['bkt_state'])
		.set('unsat_state', bkt_stateName2StateId['unsat_state'])
};
...
states.set(bkt_state.id, bkt_state);
...
export const conflict = bkt_state.id;
